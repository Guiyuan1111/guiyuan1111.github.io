...
export type MusicPlayerConfig = {
	...
	meting_api?: string; // Meting API 地址（local模式下可选）
	id?: string; // 歌单ID（local模式下可选）
	server?: string; // 音乐源服务器（local模式下可选）
	type?: string; // 音乐类型（local模式下可选）
	local?: {
		songs: Array<{
			title: string;
			artist: string;
...
export type SakuraConfig = {
	...
	image?: string; // 飘落物体图片路径，默认为樱花图片
	textParticles?: {
		enable: boolean; // 是否启用文字粒子
		texts: string[]; // 要显示的文字列表
		num: number; // 文字粒子数量
		color?: string | string[]; // 文字颜色，支持单个颜色或颜色数组（随机选择）
		font?: string; // 文字字体，默认为ZenMaruGothic-Medium
	};
};
